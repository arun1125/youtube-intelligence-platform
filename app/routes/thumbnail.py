        ...
        try:
            ...
            if test_response.data and len(test_response.data) > 0:
                ...
                if test_videos_response.data:
                    ...
                    recent_test_html = templates.get_template("components/video_grid.html").render(
                        videos=videos,
...
async def view_preview(
    ...
    try:
        ...
        # Render the grid HTML
        video_grid_html = templates.get_template("components/video_grid.html").render(
            videos=videos,
            video_count=len(videos),
            session_id=test_id
        )
    ...
    try:
        ...
        if unique_channels:
            ...
            try:
                ...
                # If RLS blocks insert, channels might already exist - continue anyway
                logger.warning(f"Could not upsert channels (might already exist): {e}")
                pass
        ...
        try:
            ...
            logger.warning(f"Could not upsert user channel: {e}")
