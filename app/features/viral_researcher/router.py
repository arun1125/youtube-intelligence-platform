...
from .creator_profile_service import CreatorProfileService
from .viral_video_service import ViralVideoService
from .transcript_service import TranscriptService
from .angle_generator_service import AngleGeneratorService
from .research_service import ResearchService
from .research_synthesis_service import ResearchSynthesisService
...
from .angle_cache_service import AngleCacheService
...
async def viral_researcher_home(request: Request):
    ...
    return request.app.state.templates.TemplateResponse(
        "viral_researcher/viral_researcher_home.html",
        {
            "request": request,
            "user": user,
            "channels": channels,
...
async def scrape_channels(
    ...
    return request.app.state.templates.TemplateResponse(
        "viral_researcher/components/scrape_results.html",
        {
            "request": request,
            "results": results
        }
...
async def list_videos(request: Request, channel_id: str, bucket: Optional[str] = None):
    ...
    return request.app.state.templates.TemplateResponse(
        "viral_researcher/viral_videos_list.html",
        {
            "request": request,
            "user": user,
...
async def video_details(request: Request, video_id: str):
    ...
    return request.app.state.templates.TemplateResponse(
        "viral_researcher/video_details.html",
        {
            "request": request,
            "user": user,
            "video": video,
...
async def generate_angles(request: Request, video_id: str):
    ...
    return request.app.state.templates.TemplateResponse(
        "viral_researcher/components/angle_list.html",
        {
            "request": request,
            "angles": angles,
            "video_id": video_id
...
async def angle_selection(request: Request, video_id: str, angle_index: int):
    ...
    return request.app.state.templates.TemplateResponse(
        "viral_researcher/angle_selection.html",
        {
            "request": request,
            "user": user,
            "video": video,
...
async def generate_script(
    ...
    try:
        ...
        # Get all angles from cache to store with the script
        all_angles = cache_service.get_angles(video_id) or [angle]

        script_data = {
            'user_id': user['user_id'],
            'original_video_id': video_id,
            'selected_angle': angle.get('angle_name'),
            'angle_options': json.dumps(all_angles),  # Store all generated angles
            'script': result['script'],
            'titles': result['titles'],
            'thumbnail_descriptions': result['thumbnails'],
            'hook_options': result.get('hook_options', []),  # Alternative hooks
...
async def view_script(request: Request, script_id: int):
    ...
    # Get hook_options from script_data (if available)
    hook_options = script_data.get('hook_options', [])
    if isinstance(hook_options, str):
        try:
            hook_options = json.loads(hook_options)
        except:
            hook_options = []

    # Use stored duration if available, otherwise calculate
    stored_duration = script_data.get('estimated_duration', '')
    display_duration = stored_duration if stored_duration else f"{est_minutes} min"
    ...
    script_context = {
        **script_data,
        ...
        'estimated_duration': display_duration,
        'created_ago': format_time_ago(script_data.get('created_at', '')) if script_data.get('created_at') else "Just now",
    ...
    return request.app.state.templates.TemplateResponse(
        "viral_researcher/script_output.html",
        {
            "request": request,
            "user": user,
...
async def my_scripts(request: Request):
    ...
    return request.app.state.templates.TemplateResponse(
        "viral_researcher/my_scripts.html",
        {
            "request": request,
            "user": user,
            "scripts": scripts
...
from .hook_library_service import HookLibraryService


@router.get("/hooks", response_class=HTMLResponse)
async def hook_library(request: Request, category: Optional[str] = None, search: Optional[str] = None):
    """Display the hook library with search and filter."""
    user = await require_creator_profile(request)

    hook_service = HookLibraryService()

    # Get hooks with optional filters
    hooks = hook_service.get_hooks(
        user_id=user['user_id'],
        category=category,
        search=search
    )
    ...
    category_counts = hook_service.get_categories_with_counts(user['user_id'])

    return request.app.state.templates.TemplateResponse(
        "viral_researcher/hook_library.html",
        {
            "request": request,
            "user": user,
            "hooks": hooks,
...
@router.post("/hooks/save")
async def save_hook(
    request: Request,
    hook_text: str = Form(...),
    category: str = Form("other"),
    source_script_id: Optional[int] = Form(None),
    source_video_title: Optional[str] = Form(None),
    source_angle: Optional[str] = Form(None)
):
...
@router.post("/hooks/{hook_id}/favorite")
async def toggle_hook_favorite(request: Request, hook_id: str):
    """Toggle favorite status of a hook."""
    user = await require_creator_profile(request)

    hook_service = HookLibraryService()
    hook = hook_service.toggle_favorite(user['user_id'], hook_id)
    ...
    if hook:
        ...
        return JSONResponse(
            content={'success': False, 'error': 'Hook not found'},
            status_code=404
        )
...
async def delete_hook(request: Request, hook_id: str):
    ...
    hook_service = HookLibraryService()
    success = hook_service.delete_hook(user['user_id'], hook_id)

    if success:
        return JSONResponse(content={'success': True})
    else:
        return JSONResponse(
            content={'success': False, 'error': 'Failed to delete hook'},
            status_code=500
        )
...
async def update_hook(
    ...
    if hook:
        return JSONResponse(content={'success': True, 'hook': hook})
    else:
        return JSONResponse(
            content={'success': False, 'error': 'Hook not found'},
            status_code=404
